...
* время развертывания для тестирования (на встраиваемом оборудовании)
...
Очереди НЗР (как и большинство очередей) *увеличивают среднее время цикла* и снижают поставку ценности и тем самым могут сократить общую прибыль.
...
* Очереди НЗР являются *полуготовой продукцией* (спецификациями, кодом, документацией, ...) с вложенными в них временем и деньгами, которые ещё не имеют возврата на инвестиций.
...
Итак, вместо более простой системы с поэлементным пополнением по модели М/М/1/∞ (на вход поступают отдельные, атомарные элементы работы), рассмотрим систему М[x]/М/1/∞ (на вход которой поступают пакеты элементов). Эта модель является лучшей аналогией традиционной продуктовой разработке.
...
Оба подмножества "текущий релиз" и "будущий бэклог" могут содержать уточнённые и неясные элементы. В начале релизного цикла, "текущий релиз" как правило содержит, в основном, неясные элементы, и Спринт за Спринтом они уточняются, пока не станут уточнёнными, а затем реализованными.

Это подводит к некоторым ключевым моментам:

* В LeSS принято --- и рекомендуется --- приоритезировать только уточнённое подмножество "текущего релиза".
* В LeSS, такая “уточнённая приоритезированная очередь” является критически важной очередью работы, стоящей перед командами.
...
* количество элементов в уточнённой приоритезированной очереди "текущего релиза"
...
* скорость поступления элементов в уточнённую приоритезированную очередь текущего релиза
...
Эта проблема менее пагубна в *приоритезированной очереди* Бэклога Продукта, поскольку он может быть пересортирован --- только что добавленный элемент может быть перемещён вверх списка. Но, несмотря на это, существуют веские причины ограничивать количество элементов в уточнённой приоритезированной очереди текущего релиза:

* Длинный список уточнённых комплексных историй тяжело понять и приоритезировать. В крупномасштабной разработке продуктов, мы регулярно слышим жалобы Владельца Продукта, что бэклог слишком большой, чтобы осознать его.
